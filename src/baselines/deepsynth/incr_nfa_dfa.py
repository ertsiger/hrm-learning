#######################################################################
# Generate NFA incrementally from trace
# Provides a module to convert NFA to corresponding DFA
...
def get_seq_input_uniq(var, input_dict):
    event_id = input_dict['event_id']

    len_seq = 2

    seq_input = var['seq_input_uniq'];
...
# def plot_model(model, trace_input, num_states, hyperparams, nfa=True, iter_num=0):
...
def parse_args():
    ...
    parser.add_argument('-o','--order', metavar = 'TRACE_ORDER', default='same', choices=['bts','stb','random','same'],
            help='Trace order')
    parser.add_argument('--incr', metavar = 'INCREMENTAL_SAT', default=True, const=True, nargs='?',
            help='Call SAT incrementally')

    hyperparams, _ = parser.parse_known_args()
    return hyperparams

def make_model(full_events, model, var, hyperparams, num_states, input_dict, start_time):
    ...
    get_seq_input_uniq(var,input_dict)
...
def convert_to_dfa_plot(full_events, model_gen, input_dict, num_states, hyperparams, var, start_time, iter_num):
...
full_path = abspath(__file__).replace('incr_nfa_dfa.py','')
C_gen_model = full_path + f'aux_files/gen_model_new_{random.randint(0, 100000000)}.c'
C_gen_model_output = full_path + f'aux_files/cbmc_output_gen_model_new_{random.randint(0, 100000000)}.json'
